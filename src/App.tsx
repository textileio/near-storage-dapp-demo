      ...
      <Row>
        <Col>
          {!currentUser 
            ...
            : !locked 
            ? <div className={`currentRule active`} onClick={mint}>{`Rule ${ruleN} — Click to lock funds to store.`}</div>
            : owned
            ? <div className={`currentRule active ${artwork ? "" : " blink"}`} onClick={mint}>{`Rule ${ruleN} — ${artwork ? "Mint new" : "Waiting to render.."}.`}</div>
